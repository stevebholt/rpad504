...
The Census Bureau has created a wealth of ESRI format shapefiles for many geogrphic units in the United States. Often a shapefile you need can be found using a simple search for what you're trying to map (e.g., New York) and shapefile, e.g., "New York shapefile." For instance, New York's can be found [here](https://gis.ny.gov/civil-boundaries) from New York state government sources or [here](https://catalog.data.gov/dataset/tiger-line-shapefile-2016-state-new-york-current-county-subdivision-state-based) from Census sources. We will use the Census sources Downloading the `.zip` folder with the shapefile will reveal a whole host of files:
...
After we remove duplicates in our datafile, we should have 4,812 observations. Now we're ready to start mapping! We will use the unique school identifier we created in our previous step (grouping the observations by `name`, `street`, and `ncessch` or the school name, street address, and NCES identifier).

::: callout-note
This is not an ideal data structure because we will be relying on the awful `m:m` merge that we should avoid in nearly all cases. A better shapefile would have been one using New York school districts as the organizing unit. However, our county map will suffice for illustrating map creation even in suboptimal circumstances.
:::
